...
```
3. Проверки dasbord  что система перестала видеть реплику и мастер
...
**Ожидаемые результаты:**
Ожидается,  что при потери соединения с узлами etcd  будут проводит выборы мастера, как только порты снова станут открытыми etcd назначит master и кластер будет работать в штатном режиме 
...
**Анализ результатов:**
Опираясь на графики было установлено, что система в  течении 10 секунд приходит к рабочему состоянию после включения портов и после задержки от 0.15 - 1с  начинает репликацию данных
...
**План:**
- Отключения реплики на длительный период
...
**Реальные результаты:**
После отключении и включения, реплика  начала получать данный от мастера,lag который возник при включении оказался не существенным, так как основная доля запросов которая летит в api - get запросы. 
...
**Ожидаемые результаты:**
Ожидается, что после включения etcd не получится корректного смены mater  и replica
